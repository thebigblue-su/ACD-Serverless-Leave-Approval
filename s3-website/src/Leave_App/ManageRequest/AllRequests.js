  ...
  return <section className="table-responsive mt-4">
    <table className="table text-center table-borderless  table-hover">
      ...
      <tbody className="table--body text-capitalize">
        {
          (data && data.log) && data.log.map((items, i) => {
            const { reason, from, to, numberOfDays, dateTime, approvalStatus } = items;
            return <tr key={i}>
              <td><b>{i + 1}</b></td>
              <td>{reason}</td>
              <td>{from}</td>
              ...
              <td className={`${approvalStatus === 'Auto' ? 'text-info' : 'text-success'}`}>
                <b>{approvalStatus}</b>
              </td>
            </tr>
          })
